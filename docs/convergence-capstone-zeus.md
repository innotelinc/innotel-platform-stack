...
**Status: proposal for review** · updated September 2026
...
|---|---|---|
| Portal | Next.js 16 (`src/app`) | Customer portal + API routes (admin, billing, contacts, fax, messages, phone, voicemail, webhooks, AMI, health) |
| PBX | FreePBX (Asterisk) via `pbx` compose service | SIP routing, extensions |
| Numbers | VoIP.ms API | Instant number provisioning |
| Softphone | SIP.js / WebRTC, WSS via `ws.<domain>` | In-browser + PWA dial pad |
| Messaging | portal API | SMS + unified messaging |
| Fax | AvantFax | Digital faxing + history |
| AI | voicemail transcription/summaries | Voicemail intelligence |
| Billing | Stripe + reseller white-label | Revenue (consumes Magnate) |
| Identity | Authentik (platform compose) | SSO |
| Ops | `scripts/` (setup, npm-proxy-hosts, seed, migrations), NPM wildcard TLS | Deploy + edge |
...
| Send SMS / fax from an agent | Portal API | `POST /api/messages`, `POST /api/fax` on the Zeus portal |
...
| Full PBX entrypoint (`pbx/` + ARI wiring scripts) | ⚠️ FreePBX only via compose | add `pbx/`-style bootstrap: extensions, routes, AMI user, WSS cert wiring |
| Systemd units for bare-metal | ❌ none | port `systemd/` units (portal, pbx-sync) |
| Ansible playbook for bare-metal PBX | ❌ none | port `ansible/dograh-ari.yml` pattern → `zeus-ari.yml` |
| Offline + live-USB ISO deploy | ❌ none | reuse `scripts/build-live-usb.sh` / `build-offline-bundle.sh` |
| Control Center ops dashboard (services/health/ports/alerts) | ⚠️ portal `/api/health` only | extend portal health page or add dashboard surface |
| Observability (OTel → SigNoz) | ❌ none | optional `compose.observability.yml` profile |
| Smoke tests (`smoke-test.sh` / `smoke-e2e.sh`) | ❌ none | port smoke suite against portal + PBX |
| Infisical profile + setup | ✅ | same contract; add `infisical://` runtime resolution (like Cerulean/Onyx) |
...
- [ ] Zeus: publish portal API spec for `messages` / `fax` / `voicemail` so
      Capstone can build against it

**Phase 1 — Zeus parity (structural mirror)**
- [ ] `pbx/` bootstrap for Zeus's FreePBX (extensions, routes, AMI user, WSS)
- [ ] `systemd/` units + optional Ansible playbook
- [ ] Smoke suite (`scripts/smoke-*.sh`) for portal + PBX health
- [ ] Control Center surface (extend portal `/dashboard/health` → ops view)
- [ ] Optional OTel → SigNoz profile
...
- **Billing** stays Magnate (both consume; Zeus's Stripe flows remain a Zeus
  feature, not a Magnate replacement — revisit when Magnate's billing API is
  production-ready).
