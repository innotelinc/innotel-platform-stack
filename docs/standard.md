...
3. **`guard-lib`** — `.githooks/guard-lib` is the single source of the policy patterns and is
   ...
   `scripts/conform-project.sh --print-guard` prints the copy the conform script installs,
   and the conformity audit reports `guard-lib is current` only when a repo's file is
   byte-identical to it — resembling the policy is not carrying it.
...
- **A verdict must not depend on the size of the input — a gate fails closed.** The rule this
  comes from: the guard used to grep `printf '%s\n' "$input" | grep -q …`, and under
  `set -o pipefail` (which `commit-msg`, `pre-commit` and the CI job all set) a `printf` writing
  more than a pipe buffer into a `grep -q` that quits at the first match dies of SIGPIPE — 141
  from the pipeline, which `if …; then` reads as *no match*. A large staged diff, a merged
